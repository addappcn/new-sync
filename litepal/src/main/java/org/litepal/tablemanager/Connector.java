...
public class Connector {
	...
	public synchronized static SQLiteDatabase getWritableDatabase() {
		...
		return litePalHelper.getWritableDatabase(CipherUtil.aesKey);
	}

	public synchronized static void rekey(String newPassphrase){
		getWritableDatabase().rawExecSQL(String.format("PRAGMA rekey = \"%s\";", newPassphrase));
		CipherUtil.aesKey = newPassphrase;
	}
	...
	public synchronized static SQLiteDatabase getReadableDatabase() {
		LitePalOpenHelper litePalHelper = buildConnection();
		return litePalHelper.getReadableDatabase(CipherUtil.aesKey);
	}
	...
	public static void clearLitePalOpenHelperInstance() {
        if (mLitePalHelper != null) {
            mLitePalHelper.getWritableDatabase(CipherUtil.aesKey).close();
            mLitePalHelper = null;
        }
	}
